...
#   - Reward scalpeur actif (bonus activité, pénalité vol*exposition)
#   - Position sizing dynamique via ATR_14
#   - Action masking single-head (6 actions, risk_scale 1x / 1.8x)
#   - Timeframe supérieur H1 (features seulement, PAS de filtre tendance)
#   - SAINTv2 allégé (sans CosinePositionalEncoding)
#   - Early stopping sur Calmar ratio
...
N_ACTIONS = 6  # 0:BUY1,1:SELL1,2:BUY2,3:SELL2,4:CLOSE,5:HOLD
...
class PPOConfig:
    ...
    # PPO Training (calibré 3070 laptop)
    epochs: int = 618
    episodes_per_epoch: int = 3
    episode_length: int = 256
    updates_per_epoch: int = 6        # <- plus d'updates pour plus de trades

    batch_size: int = 256             # <- batch plus gros = politique plus stable
    gamma: float = 0.97
    ...
    entropy_coef: float = 0.022       # <- un peu plus d'entropie
    ...
    # Risk management / position sizing (agressif mais contrôlé)
    risk_per_trade: float = 0.009     # <- 0.9 % du capital par trade
    ...
    scalping_holding_penalty: float = 2.5e-5  # plus utilisé dans le reward final (on garde dans la config)
...
class MarketData:
    def __init__(self, df: pd.DataFrame, feature_cols: List[str],
        ...
        # Données brutes utiles pour risk management / H1
...
class BTCTradingEnvDiscrete(gym.Env):
    ...
    Actions agent (single-head + mask) :
        0 = BUY  (risk 1x)
        1 = SELL (risk 1x)
        2 = BUY  (risk 1.8x)
        3 = SELL (risk 1.8x)
      ...
      - si position == 0 :
            actions valides = {0,1,2,3,5} (CLOSE masqué)
      - si position != 0 :
            actions valides = {4,5} (ouvertures masquées)
    """
    ...
    def __init__(self, data: MarketData, cfg: PPOConfig):
        ...
        self.action_space = spaces.Discrete(4)  # interne
    ...
    def _force_close(self, price: float) -> float:
        if self.position == 0 or self.current_size <= 0:
            return 0.0
    ...
    def _compute_dynamic_size(self, price: float) -> float:
        ...
        # Stop basé sur ATR_14 brut
        atr = float(self.data.atr14[self.idx - 1]) if self.idx - 1 >= 0 else 0.0
        stop_distance = atr * 1.5
        stop_floor = 0.0015 * price
        stop_distance = max(stop_distance, stop_floor, 1e-8)

        risk = self.cfg.risk_per_trade * max(self.risk_scale, 1e-6)

        size = risk * self.capital / (stop_distance * self.cfg.leverage + 1e-8)
    ...
    # NOTE : plus de filtre tendance H1, l'agent décide seul.

    def step(self, action: int):
        """
        action (env) :
          0 = BUY, 1 = SELL, 2 = CLOSE, 3 = HOLD
        """
        price = self.data.close[self.idx]
        old_pos = self.position
        prev_capital = self.capital
        realized = 0.0  # PnL réalisé (pour info / debug)

        # --------- exécution ---------

        # Fermeture manuelle
        if action == 2 and old_pos != 0:
            exec_price = self._apply_micro(price, -old_pos)
        ...
        elif action in (0, 1) and old_pos == 0:
        ...
        # Mise à jour du temps en position
        if old_pos != 0 and self.position != 0:
            self.bars_in_position += 1
        elif old_pos == 0 and self.position != 0:
            self.bars_in_position = 1
        else:
            self.bars_in_position = 0

        # Fermeture forcée max holding
        if self.position != 0 and self.bars_in_position > self.cfg.scalping_max_holding:
            self._force_close(price)

        # --------- REWARD SHAPING FINAL "LOUP QUI LAISSE COURIR" ---------

        delta_cap = self.capital - prev_capital
        ret = delta_cap / self.cfg.initial_capital

        # Base : retour absolu
        reward = ret

        # Pénalité pertes modérée (mais non négligeable)
        if ret < 0:
            reward *= 2.6

        # Bonus d'activité constant : pousse l'agent à agir
        reward += 1.2e-5

        # Volatilité récente (fenêtre un peu plus large)
        window_start = max(self.idx - 30, self.start_idx)
        if self.idx > window_start:
            recent_slice = self.data.close[window_start:self.idx]
            recent_vol = float(np.std(recent_slice))
        else:
            recent_vol = 0.0

        # Exposition (en % du capital)
        if self.position != 0 and self.current_size > 0 and self.capital > 0:
            exposure = abs(self.position) * self.current_size * price / self.capital
        else:
            exposure = 0.0

        # Malus vol × exposition
        reward -= 8e-5 * exposure * recent_vol / (price + 1e-8)

        # Latent PnL pour bonus "laisser courir" et equity
        latent = 0.0
        ...
        # Malus holding très doux (seulement après 8 bougies en position)
        if self.position != 0:
            reward -= 3e-6 * max(0, self.bars_in_position - 8)

        # BONUS : laisser courir les gagnants
        if self.position != 0 and self.current_size > 0 and self.entry_price > 0:
            unreal_denom = self.current_size * self.entry_price * self.cfg.leverage + 1e-8
            unreal_pct = latent / unreal_denom
            if unreal_pct > 0.003 and self.bars_in_position > 5:
                # bonus croissant avec le temps passé en gain
                reward += 1.5e-5 * (self.bars_in_position - 5)

        # --------- DRAWDOWN SUR EQUITY + MAX_DD ---------

        equity = self.capital
        if self.position != 0 and self.current_size > 0:
            equity += latent
...
class SAINTPolicySingleHead(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        tok = self.val_proj(x.unsqueeze(-1))  # (B,T,F,D)
        ...
        # agrégation : moyenne sur le temps + sous-ensemble de features
        F_start = min(9, F // 4)
        F_end = min(F, F_start + 5)
        if F_start < F_end:
            tok_sub = tok[:, :, F_start:F_end, :]
        else:
            tok_sub = tok

        h = tok_sub.mean(dim=(1, 2))
        h = self.norm(h)
        h = self.mlp(h)
...
def epsilon_greedy_from_logits(logits: torch.Tensor, eps: float) -> int:
    with torch.no_grad():
...
def build_mask_from_pos_scalar(pos: int, device) -> torch.Tensor:
    ...
    if pos == 0:
        ...
        # en position -> seulement CLOSE & HOLD
        mask[0:4] = False
        mask[4] = True
        mask[5] = True
    return mask
...
def build_action_mask_from_states(states: torch.Tensor) -> torch.Tensor:
    ...
    if flat.any():
        mask[flat] = True
        mask[flat, 4] = False  # pas de CLOSE en flat
...
def run_training(cfg: PPOConfig):
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        action_counts_env = np.zeros(4, dtype=np.int64)
        ...
        for ep in range(cfg.episodes_per_epoch):
            ...
            while not done:
                ...
                with torch.no_grad():
                    logits, value = policy(s)  # logits : (1, N_ACTIONS)
                    logits = logits[0]
                    ...
                    # -------- mapping agent -> env + risk_scale (1x / 1.8x) --------
                    if pos == 0:  # flat
                        if a in (0, 2):      # BUY 1x / 1.8x
                            env_action = 0
                ...
                # normalisation online du reward
                reward_count += 1
                delta = reward - reward_mean
                reward_mean += delta / max(reward_count, 1)
                delta2 = reward - reward_mean
                reward_var = 0.99 * reward_var + 0.01 * delta2 * delta
                reward_std = math.sqrt(max(reward_var, 1e-8))
                norm_reward = reward / (reward_std + 1e-8)
                ep_rewards_norm.append(norm_reward)

                ep_states.append(s.cpu().numpy())
                ep_actions.append(a)
            ...
            adv, ret = compute_gae(
                ep_rewards_norm, ep_values, ep_dones,
        ...
        # --------- tenseurs batch ---------
        states = torch.tensor(np.vstack(batch_states), dtype=torch.float32, device=device)
        ...
        for upd in range(cfg.updates_per_epoch):
            ...
            for start in range(0, n_samples, cfg.batch_size):
                ...
                with torch.amp.autocast(device_type=device.type, enabled=cfg.use_amp):
                    ...
                    v_clipped = val_old + (value - val_old).clamp(-0.2, 0.2)
                    critic_loss = torch.max(
                        (value - ret_b).pow(2),
                        (v_clipped - ret_b).pow(2)
                    ).mean()

                    # Entropy bonus dynamique : boost si la politique devient trop déterministe
                    entropy_bonus = cfg.entropy_coef * entropy
                    if entropy < 0.8:     # seuil "Loup Scalpeur"
                        entropy_bonus = entropy_bonus * 4.0
        ...
        buy_count, sell_count, close_count, hold_count = action_counts_env
        ...
        print(
            ...
            f"ENV B:{buy_ratio:4.1%} S:{sell_ratio:4.1%} C:{close_ratio:4.1%} H:{hold_ratio:4.1%}  "
            f"KL={np.mean(epoch_kl):.4f}"
        )

        # --------- early stopping sur Calmar ---------
        if calmar > best_calmar:
            ...
            epochs_no_improve = 0
            print(f"[EPOCH {epoch:03d}] Nouveau best model sauvegardé (Calmar={calmar:.3f}).")
        else:
            epochs_no_improve += 1
            if epochs_no_improve >= patience:
                print(f"Early stopping après {epoch} epochs (Calmar ne progresse plus).")
                break
